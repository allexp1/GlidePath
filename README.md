...
The app icon is the **Gateway Arc** mark: two posts, a detached arc, and a road
running to the horizon between them — a measured section, which is the one thing
this app does that a camera database does not.

`scripts/generate-app-icon.py` still exists and still draws the old placeholder,
so `make icon` refuses to run unless you pass `FORCE=1`. It would otherwise
overwrite the real artwork with a coloured square, and nothing before the
upload would complain.
